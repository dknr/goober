...
func main() {
	configPath := flag.String("config", "gbr-controld.toml", "Path to configuration file")
	flag.Parse()

	// Load configuration
	cfg, err := config.LoadControlDaemonConfig(*configPath)
	if err != nil {
	...
	logger.Info("Starting gbr-controld...")
